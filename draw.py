...
def generate_sphere( cx, cy, cz, r, step ):
    semicircle = []
    whole_sphere = []
    rot_matrix = make_rotX(360.0 / step)
    for point in range(0, step + 1):
        semicircle.append([r * math.cos(math.pi * point / step), r * math.sin(math.pi * point / step), 0, 1])

    for semi_num in range(0, step):
        whole_sphere.extend(deepcopy(semicircle))
        matrix_mult(rot_matrix, whole_sphere)
        
    translate_mat = make_translate(cx, cy, cz)
    matrix_mult(translate_mat, whole_sphere)
        
    return whole_sphere

def add_torus( points, cx, cy, cz, r0, r1, step ):
    torus_points = generate_torus(cx, cy, cz, r0, r1, step)
    for point in torus_points:
        add_edge(points, point[0], point[1], point[2], point[0] + 1, point[1], point[2])
        
def generate_torus( cx, cy, cz, r0, r1, step ):
    whole_torus = []
    rot_matrix = make_rotY(360.0 / step)

    for circle_num in range(0, step):
        add_circle(whole_torus, r1, 0, 0, r0, step)
        matrix_mult(rot_matrix, whole_torus)
    
    translate_mat = make_translate(cx, cy, cz)
    matrix_mult(translate_mat, whole_torus)
